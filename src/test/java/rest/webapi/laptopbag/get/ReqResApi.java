...
public class ReqResApi {
	...
	public void JsonPathParactice() throws URISyntaxException {
		...
		/*
		 * Given accept type is json When i perform GET request using ALL method
	...
	public void XmlPathSamples() throws URISyntaxException {
		/*
		 * Given Accept type is xml When i perform GET request to get all
		 * laptops Then status is 200 And I can navigate through XML using
		 * XmlPath
		 */
		URI uri = new URI(RestHelper.buildUri(Methods.ALL)); // new
																// URI("http://localhost:8085/laptop-bag/webapi/api/all");
		Response response = given().accept(ContentType.XML).when().get(uri);
